...
if [ "$#" -ne 2 ]; then
    ...
    echo "Example: $0 /path/to/root 1" >&2
    exit 2
fi
...
stripScriptPath="./s25-scripts/leafNameStripper.py"

# Files used by runApro3
# dupLossOutputFile="${n}_gt_apro-cleaned.tre"
# discoNoDecomp="${n}-gt-disco-rooted.tre"

input_file="e${n}00-cleaned.tre"
output_file="e${n}00_stripped.tre"
...
stripDupSuffix(){
    for file in $(ls "$root"); do
        if [[ -d "$root/$file" ]]; then
            ...
            # cat "$root/$file/$input_file"
            python3 "$stripScriptPath" -i "$root/$file/$input_file" -o "$root/$file/$output_file"
        fi
    done
}
