...
for cond in "${model_conditions[@]}"; do
    ...
    mkdir -p "simulated-new/species-rax-outputs/$cond"
    bash run-speciesrax/run-one-model-cond.sh "simulated-new/species-rax-family-files/$cond" "simulated-new/species-rax-outputs/$cond" > "$log_folder/$cond/run_output.log" 2> "$log_folder/$cond/run_error.log"
