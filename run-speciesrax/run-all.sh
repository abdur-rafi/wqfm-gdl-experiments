...
# List of model conditions to process (copied from families-file-generator-all.sh)
model_conditions=(
        sim200_dup3_loss1
        sim200_dup0_ILS70
        sim500_dup1_loss0.1
...
# Determine script directory and path to the model-condition runner
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
model_cond_script="$script_dir/run-one-model-cond.sh"

if [ ! -f "$model_cond_script" ]; then
    echo "Error: run-one-model-cond.sh not found: $model_cond_script" >&2
    exit 1
fi
