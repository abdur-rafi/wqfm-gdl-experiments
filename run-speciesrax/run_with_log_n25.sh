log_parent_folder=25_taxa_separated_trees/log
...
for cond in "${model_conditions[@]}"; do
    ...
    mkdir -p "25_taxa_separated_trees/species-rax-outputs/$cond"
    bash run-speciesrax/run-one-model-cond_n25.sh "25_taxa_separated_trees/species-rax-family-files/$cond" "25_taxa_separated_trees/species-rax-outputs/$cond" > "$log_folder/$cond/run_output.log" 2> "$log_folder/$cond/run_error.log"
