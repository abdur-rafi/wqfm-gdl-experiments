...
usage() {
    echo "Usage: $0 <root_dir> <out_dir> <link_dir>"
    echo "Example: $0 /path/to/simulations /path/to/output /path/to/link"
    exit 2
}
...
if [ ! -d "$link_dir" ]; then
    echo "Error: link_dir not found or not a directory: $link_dir" >&2
    exit 1
fi
...
# List of model conditions to process (copied from mapping-file-generator-all.sh)
model_conditions=(
        sim200_dup3_loss1
        sim200_dup0_ILS70
        sim500_dup1_loss0.1
...
for model_cond in "${model_conditions[@]}"; do
    ...
    bash "$model_cond_script" "$model_cond_path" "$out_model_dir" "$link_dir/$model_cond"
done
