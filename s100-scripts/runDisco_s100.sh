...
geneTreeLabelCleaned="g_trees-raxml-sqlen-${n}-cleaned.tre"
...
discoOutput=g_trees-raxml-sqlen-${n}-disco-decomp.tre
...
discoCleaned=g_trees-raxml-sqlen-${n}-disco-decomp-cleaned.tre
...
discoNoDecomp=g_trees-raxml-sqlen-${n}-disco-rooted.tre
...
discoNoDecompCleaned=g_trees-raxml-sqlen-${n}-disco-rooted-cleaned.tre
...
consensusTree="cons-${n}.greedy.tree"
...
outputFile="g_trees-raxml-sqlen-${n}-wqfm-tree.tre"

outputFileApro="merged_gene_trees-apro.tre"
outputFileApro="g_trees-with-gt3species-apro.tre"
outputFileApro="g_trees-raxml-sqlen-${n}-apro.tre"
...
wqfm2020OutputFile="g_trees-raxml-sqlen-${n}-wqfm-2020.tre"


wqfm2020OutputFileTrue="true-wqfm-2020.tre"

# quartetFile=sqQuartetsNormedPerTree.txt
quartetFile="${n}-sqQuartets.txt"
...
wscores="${n}-wqfm-tree-scores.txt"

ascores="apro-3-scores.txt"
ascores="${n}-apro-scores.txt"
...
w2020scores="${n}-wqfm-2020-scores.txt"
...
quartetGenJarPath="/mnt/disk1/wqfm-tree-stuff/experiments/genSQ.jar"
wqfmPath="/mnt/disk1/wqfm-tree-stuff/experiments/wQFM-v1.4/wQFM-v1.4.jar"
# create a function
...
runWqfm2020(){
    for file in $(ls $root); do
        if [ -d $root/$file ]; then
            ...
            java -jar "$wqfmPath" -i $root/$file/$quartetFile -o $root/$file/$wqfm2020OutputFile
...
generateSqQuartets(){
    ...
    # check if $file is a directory
        if [ -d $root/$file ]; then
            ...
            java -jar -Xmx60g "$quartetGenJarPath" $root/$file/$discoNoDecompCleaned > $root/$file/$quartetFile
