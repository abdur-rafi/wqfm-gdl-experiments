        ...
        for line in infile:
            ...
            for original_label in sorted(set(labels_in_tree)): # Process unique labels
                if original_label not in original_to_short_map:
                    short_label = f"G__{short_label_counter}"
...
def convert_back_to_original(input_relabeled_tree_file, map_file, output_original_tree_file):
    ...
    # import json
    # json.dump(short_to_original_map, open("short_to_original_map.json", 'w'), indent=4)
...
if __name__ == "__main__":
    # --- Configuration ---
    input_gene_trees_file = "Archaea364/364_combined_treefile.txt"
    tree="top25"
    input_gene_trees_file = f"Archaea364/{tree}_gt.tree"

    output_relabeled_trees_file = "Archaea364/top25_relabeled_gene_trees.txt"
    output_relabeled_trees_file = f"Archaea364/{tree.split('.')[0]}_relabeled.txt"

    output_label_map_file = "Archaea364/364_gene_label_map.tsv"
    output_label_map_file = f"Archaea364/{tree.split('.')[0]}_label_map.tsv"

    print(f"output_label_map_file: {output_label_map_file}")    
    
    output_original_trees_file = "Archaea364/364_converted_back_trees.txt" # For verification
    output_original_trees_file = f"Archaea364/decomposed_renamed/{tree.split('.')[0]}_converted_back.txt"

    convert_back_input_file = f"Archaea364/outputs/{tree}_wqfm_tree_pro.tre"
    # convert_back_input_file = f"Archaea364/outputs/{tree}_apro.tre"

    # convert_back_input_file = f"Archaea364/decomposed_renamed/{tree.split('.')[0]}_relabeled.txt"

    converted_back_file = f"Archaea364/outputs/{tree}_wqfm_tree_pro_true_label.txt"
    # converted_back_file = f"Archaea364/outputs/{tree}_apro_true_label.txt"
    
    # converted_back_file = f"Archaea364/decomposed_renamed/{tree.split('.')[0]}_original.txt"
